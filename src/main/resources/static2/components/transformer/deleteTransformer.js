define(['vue','text!./deleteTransformer.html'], function (Vue,componentTemplate) {	
	let componentProperties = {
		    ...
		    methods: {
		    	updateRouterView:function(path,query) {
		       	...
		       	back: function () {	
		       		this.hide();		       	
		       		$('#deleteTransformerModal').modal('hide');
		       	},
		       	deleteTransformer: function () {			       		
		       		//发送请求告知服务器
		       		this.hide();			       		
		       		let tmpVue=this;		       		
		       		for(let item of this.query.items){
			       		let url=this.$store.state.BASE_PATH+"/api/github/qq275860560/transformer/deleteTransformer?id="+item.id;
						...
						fetch(url,{method:"GET", mode:"cors",headers:{"Authorization": token_type+" "+access_token }					
							...
							tmpVue.code=result.code;
							 if(result.code==200){
								   ...
								   tmpVue.$emit('pageTransformer',tmpVue.query.pageNum,tmpVue.query.pageSize);							
		    	...
		    	show:function (query){
		    		...
		    		$('#deleteTransformerModal').modal('show');
		...
		return Vue.component('deleteTransformer', componentProperties);	
}); 